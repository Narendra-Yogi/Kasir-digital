            ...
            <div class="pt-4 border-t border-gray-100 space-y-4">
                ...
                <div>
                    ...
                    <select name="security_question" class="w-full px-4 py-3 rounded-xl border border-gray-200 bg-gray-50 focus:bg-white focus:border-brand-500 outline-none cursor-pointer" required>
                        ...
                        @php $sq = old('security_question', $user->security_question); @endphp
                        <option value="Siapa nama hewan peliharaan pertama Anda?" {{ $sq == 'Siapa nama hewan peliharaan pertama Anda?' ? 'selected' : '' }}>Siapa nama hewan peliharaan pertama Anda?</option>
                        <option value="Di kota mana Anda dilahirkan?" {{ $sq == 'Di kota mana Anda dilahirkan?' ? 'selected' : '' }}>Di kota mana Anda dilahirkan?</option>
                        <option value="Siapa nama ibu kandung Anda?" {{ $sq == 'Siapa nama ibu kandung Anda?' ? 'selected' : '' }}>Siapa nama ibu kandung Anda?</option>
                        <option value="Apa nama sekolah dasar Anda?" {{ $sq == 'Apa nama sekolah dasar Anda?' ? 'selected' : '' }}>Apa nama sekolah dasar Anda?</option>
                        <option value="Apa makanan favorit Anda?" {{ $sq == 'Apa makanan favorit Anda?' ? 'selected' : '' }}>Apa makanan favorit Anda?</option>
                        <option value="Siapa nama sahabat terbaik Anda?" {{ $sq == 'Siapa nama sahabat terbaik Anda?' ? 'selected' : '' }}>Siapa nama sahabat terbaik Anda?</option>
                    </select>
                    @error('security_question') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">Jawaban Keamanan Baru (Opsional)</label>
                    <input type="text" name="security_answer" class="w-full px-4 py-3 rounded-xl border border-gray-200 bg-gray-50 focus:bg-white focus:border-brand-500 outline-none transition-all" placeholder="Kosongkan jika tidak ingin mengubah jawaban">
                    <p class="text-[11px] text-gray-400 mt-1">Jawaban tidak bersifat case-sensitive. {{ $user->security_answer ? '✅ Jawaban keamanan sudah diset.' : '⚠️ Jawaban keamanan belum diset.' }}</p>
