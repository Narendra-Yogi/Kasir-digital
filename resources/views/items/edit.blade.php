            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="price" value="{{ number_format($item->price, 0, ',', '.') }}" class="w-full px-4 py-3 rounded-xl border border-gray-200 bg-gray-50 focus:bg-white focus:outline-none focus:border-brand-500 transition-colors" required oninput="formatThousandsInput(this); hitungHppOtomatis()">
                    </div>
                ...
                <div class="border-t border-gray-100 pt-6">
                    ...
                    <div id="ingredientContainer" class="space-y-3">
                        {{-- Bahan yang sudah tersimpan diload via JavaScript --}}
                    </div>
...
<script>
    ...
    // Data resep yang sudah tersimpan (termasuk unit_used)
    const existingIngredients = @json($item->itemIngredients->map(function($ii) {
        return ['ingredient_id' => $ii->ingredient_id, 'quantity_needed' => $ii->quantity_needed, 'unit_used' => $ii->unit_used];
    }));
    ...
    function hitungHppOtomatis() {
        ...
        rows.forEach(function(row) {
            ...
            if (select.value && qtyInput.value) {
                ...
                const costPerUnit = parseFloat(option.dataset.cost) || 0;
                const ingredientUnit = option.dataset.unit;
    ...
    // Load resep bahan yang sudah tersimpan saat halaman dimuat (termasuk unit_used)
    document.addEventListener('DOMContentLoaded', function() {
        existingIngredients.forEach(function(ei) {
            tambahBahan(ei.ingredient_id, ei.quantity_needed, ei.unit_used || '');
        });
    });
</script>
@endsection